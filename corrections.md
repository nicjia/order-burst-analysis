...
### [~] M12 — Hidden-execution finding too thin for its billing (P2) — PARTIAL (paper edits done; cross-section data-gated)
...
- **REMAINING (Phase 4, data-gated on LOBSTER re-fetch from lobster2):** (a) report fraction of hidden prints at midpoint + classification sensitivity; (b) extend to a real cross-section (≥2 yrs, more names). Requires re-fetching raw LOBSTER (data folders were deleted for quota). See recommendation below.
