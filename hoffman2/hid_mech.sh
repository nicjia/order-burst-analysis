...
# One task = one ticker. Emits results/hid_mech/out/$TK.csv.
set -uo pipefail
...
GD=results/hid_mech; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/hid_mech
  ...
  if [ -n "$msg" ]; then python3 src_py/hidden_mechanical.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,n_agg,n_far,acc1,acc5,acc30,acc180,res5,res30,far180,far_res5,far_res30,n_rf,n_nrf,rf180,nrf180,corr_ofi" > "$GD/out/$TK.csv"
