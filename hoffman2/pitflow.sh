...
# One task = one ticker. Emits results/pitflow/out/$TK.csv.
set -uo pipefail
...
GD=results/pitflow; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/pitflow
  ...
  if [ -n "$msg" ]; then python3 src_py/pit_flow.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,n_vis,vol_vis,signed_vis,dvol_vis,n_hid,vol_hid,signed_hid,open_mid,close_mid,halfsp" > "$GD/out/$TK.csv"
