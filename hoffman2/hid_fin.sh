#!/bin/bash
# hid_fin.sh — SGE array: term structure (full universe) + VAR spec grid + sequential-sweep test.
# One task = one ticker. Emits results/hid_fin/out/$TK.csv.
set -uo pipefail
...
GD=results/hid_fin; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/hid_fin
  ...
  if [ -n "$msg" ]; then python3 src_py/hidden_final.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,n_b,mk3,mk15,mk30,mk60,mk120,mkclose,pmk3,pmk15,pmk30,pmk60,pmk120,pmkclose,s1_stat,s1_3,s1_10,s1_30,s2_stat,s2_3,s2_10,s2_30,s3_stat,s3_3,s3_10,s3_30,s4_stat,s4_3,s4_10,s4_30,n_sw,fsw100,fsw1s,dsz100,dsz1s,mk3_sw,mk3_unsw" > "$GD/out/$TK.csv"
