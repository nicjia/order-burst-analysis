...
# One task = one ticker. Emits results/hidden_hs474/out/$TK.csv.
set -uo pipefail
...
GD=results/hidden_hs474; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/hidden_hs474
  ...
  if [ -n "$msg" ]; then python3 src_py/hidden_spread_decomp.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,n,eff_hs,quoted_hs,price_impact,realized_hs" > "$GD/out/$TK.csv"
