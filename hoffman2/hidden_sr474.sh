...
# One task = one ticker. Emits results/hidden_sr474/out/$TK.csv.
set -uo pipefail
...
GD=results/hidden_sr474; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/hidden_sr474
  ...
  if [ -n "$msg" ]; then python3 src_py/hidden_signrobust.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
