#!/bin/bash
# hid_sw2.sh — SGE array: non-circular sequential-sweep test.
# One task = one ticker. Emits results/hid_sw2/out/$TK.csv.
set -uo pipefail
...
GD=results/hid_sw2; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/hid_sw2
  ...
  if [ -n "$msg" ]; then python3 src_py/hidden_sweep2.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,n_agg,fsw,mkT_sw,mkT_un,mk1_sw,mk1_un,mk5_sw,mk5_un,dep_sw,dep_un,n_sw,n_un" > "$GD/out/$TK.csv"
