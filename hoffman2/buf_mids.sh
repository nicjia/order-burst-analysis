...
# One task = one ticker. Emits results/buf_mids/out/$TK.csv.
set -uo pipefail
...
GD=results/buf_mids; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/buf_mids
  ...
  if [ -n "$msg" ]; then python3 src_py/buffered_mids.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,mid_open,mid_0935,mid_1555,mid_close" > "$GD/out/$TK.csv"
