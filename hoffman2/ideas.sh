...
# One task = one ticker. Emits results/ideas/out/$TK.csv.
set -uo pipefail
...
GD=results/ideas; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/ideas
  ...
  if [ -n "$msg" ]; then python3 src_py/idea_zoo.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
