...
# One task = one ticker. Emits results/harval/out/$TK.csv.
set -uo pipefail
...
GD=results/harval; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/harval
  ...
  if [ -n "$msg" ]; then python3 src_py/idea_zoo2.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,idea,family,param,n,v3,v10,v30,net3" > "$GD/out/$TK.csv"
