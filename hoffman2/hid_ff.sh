#!/bin/bash
# hid_ff.sh — SGE array: price-free burst formation test (circularity).
# One task = one ticker. Emits results/hid_ff/out/$TK.csv.
set -uo pipefail
...
GD=results/hid_ff; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/hid_ff
  ...
  if [ -n "$msg" ]; then python3 src_py/hidden_freeform.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,nA,mkA3,mkA15,mkA30,nB,mkB3,mkB15,mkB30,nC,mkC3,mkC15,mkC30" > "$GD/out/$TK.csv"
