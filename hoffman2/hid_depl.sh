...
# One task = one ticker. Emits results/hid_depl/out/$TK.csv.
set -uo pipefail
...
GD=results/hid_depl; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/hid_depl
  ...
  if [ -n "$msg" ]; then python3 src_py/hidden_depletion.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,n_agg,mk_all,mk_d1,mk_d2,mk_d3,mk_d4,mk_d5,n_d1,n_d2,n_d3,n_d4,n_d5,mk_lo,n_lo,mk_hi,n_hi,med_d,med_depth" > "$GD/out/$TK.csv"
