...
# One task = one ticker. Emits results/hid_pre/out/$TK.csv.
set -uo pipefail
...
GD=results/hid_pre; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/hid_pre
  ...
  if [ -n "$msg" ]; then python3 src_py/hidden_preprint.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
  else echo "$TK,$dd,MISSING" > "$row"; fi
  rm -rf "$d" "$d.7z"
}
export -f work
xargs -P$PAR -n1 -I{} bash -c 'work "$@"' _ {} "$TK" < "$GD/dates.txt"
echo "ticker,date,n_agg,n_far,pre180,pre30,pre5,pre1,post180,f_pre30,f_pre5,f_post180" > "$GD/out/$TK.csv"
