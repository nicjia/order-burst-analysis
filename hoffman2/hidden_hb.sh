...
# One task = one ticker. Emits results/hidden_hb/out/$TK.csv.
set -uo pipefail
...
GD=results/hidden_hb; PAR=6; L=nicjia@lobster2.math.ucla.edu
...
work(){
  dd=$1; TK=$2; L=nicjia@lobster2.math.ucla.edu; GD=results/hidden_hb
  ...
  if [ -n "$msg" ]; then python3 src_py/hidden_hasbrouck.py --msg "$msg" --ticker "$TK" > "$row" 2>/dev/null
