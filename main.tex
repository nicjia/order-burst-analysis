...
The central finding is one of rapid, orderly information decay. Order bursts carry a genuine, measurable informed footprint at minutes-scale horizons: properly (Lee--Ready) signed hidden-execution bursts move price by a mean $+1.6$ bps over three minutes across the full 474-name cross-section (pooled date-clustered $t=3.4$, individually significant in $79\%$ of names, and surviving a matched-random-time placebo that leaves $95\%$ of the effect intact). This footprint is bounded on the two dimensions that determine tradability: it is \emph{sub-spread}---smaller than the bid--ask spread one must cross to trade it---and it \emph{does not reach the overnight horizon} (close-to-open information coefficient $-0.001$, $t=-0.4$). The directional information injected by informed bursts is thus impounded within minutes and \emph{persists} through the trading day (net $+2.4$ bps to the close under a time-of-day-stratified placebo); only across the overnight auction is it fully priced out, after which the residual relation is reversal-signed---consistent with liquidity providers absorbing the transient impact and reverting the overshoot. Splitting the daily return into its two sessions sharpens what the overnight auction does: the \emph{normalized} order imbalance carries no close-to-open information, but the \emph{magnitude} of net signed burst volume predicts the overnight return with a dollar-neutral Sharpe of $\approx\!2.4$ in both a 2017--2021 aggressive-flow panel and an independent 2023--2024 hidden-execution panel---an effect a close-to-close horizon conceals, and one that decays within a single day.

The measurement rests on two now-standard safeguards rather than novel methods: the forward-looking decay filter $D_b$ (definitionally the 1--10 minute forward markout) is excluded from the predictive feature set and from any intraday evaluation inside its own window; and inference is conducted at the day level with date-clustered errors. The paper's contribution is therefore empirical rather than methodological: a precise, cross-sectional characterization of informational order flow as a real, intraday-persistent but sub-spread phenomenon that the overnight market prices out with efficiency, together with the identification of hidden-execution clustering as the one burst reconstruction---among Hawkes, order-flow-imbalance, and book-resilience alternatives---carrying a robust informed footprint. The single conditional affirmative result---a tick-structure-governed asymmetry (significant reversal in tick-constrained names, statistically undetectable in large-tick ones) orthogonal to the Fama--French five factors plus momentum and to the short-term-reversal factor---earns a market-neutral out-of-sample Sharpe of $\approx0.8$ that is economically suggestive but does not survive multiple-testing deflation.
\end{abstract}
...
ST\_Rev  & $+0.023$ & $+1.01$ & $+0.001$ & $+0.09$ \\
\bottomrule
\end{tabular}
\caption{FF5+MOM+ST\_Rev factor regression of the tick-constrained reversal ($R^2\approx0.03$). \emph{Primary} columns are the strict walk-forward out-of-sample series (2023--2026, quarterly re-selected subset, 2022 burn-in); the alpha is orthogonal to the factors but insignificant ($t=1.42$). The full-period columns ($t=3.09$) are in-sample and shown only for comparison. The short-term-reversal factor (ST\_Rev, Ken French daily) carries an \emph{insignificant} loading and leaves the alpha unchanged, so the signal is not a repackaging of the standard one-month reversal. Annualized OOS $\alpha\approx+16\%$ at the point estimate.}
\label{tab:ff_alpha}
\end{table}
...
\textbf{Survivorship and delisting.} We quantify the exposure rather than only flag it, using the fixed-at-start membership rule of Section~\ref{sec:data} under which a mid-sample delisting would surface as a truncated price series, not a silent exclusion. Scanning the daily panel over the full 2022--2026 sample for such truncations, exactly \emph{one} of the 500 fixed-list names (SQ) has a price series ending before 2026---and that reflects Block, Inc.'s 2025 ticker change (SQ$\to$XYZ) in a large-cap, not a delisting, and the name is not in the reversal universe. \emph{Zero} of the bottom-100 reversal names terminate mid-sample, so there is no within-panel delisting to bias the estimate; assigning a conservative $-30\%$ delisting return to any putative exiter leaves the out-of-sample Sharpe unchanged. Decisively, the bottom-100 names on which the reversal lives are cheap \emph{large-caps}, not micro-caps: their time-averaged 2022--2026 price ranges from \$7.95 to \$30.7 (median \$18.0), with \emph{none} below \$5. This is a structural consequence of the liquidity-based universe, since a name needs sufficient volume to generate detectable bursts and therefore cannot be a delisting-prone penny stock, and it directly counters the usual survivorship worry for a cheap-stock reversal (names like F, AAL, SOFI, and NIO are not delisting candidates). The one residual concern the fixed-at-start rule does not fully close is \emph{pre-sample} selection---whether the start-of-sample liquidity screen implicitly omitted names already impaired in 2021; bounding it completely would require the pre-survival coverage list and a CRSP delisting-return splice \citep{shumway1997}, which the cheap-large-cap composition makes unlikely to matter but which we flag as the single remaining survivorship item.

Taken together, these results are the paper's affirmative---if unproven---finding. Filtered burst flow is informative in the sense that its sign is governed by tick structure: significant mean-reversion in tick-constrained names, weakening to a statistically undetectable effect in large-tick names (we find no significant continuation there), an asymmetry orthogonal to common factors. The associated market-neutral strategy is economically positive out of sample and robust to the choice of universe cutoff, but its out-of-sample Sharpe ($\approx0.8$) is not statistically distinguishable from zero; the significant full-sample figures are best regarded as an in-sample ceiling. One caveat tempers the inventory-management reading of this reversal: the liquidity-provision account of \citet{nagel2012} predicts that reversal profits should rise with volatility, but interacting the daily reversal return with trailing realized volatility yields an insignificant (indeed weakly negative) coefficient ($t=-1.2$), so we do not claim the volatility state-dependence that inventory models imply. The regime mechanism is thus the durable contribution, while its monetization---and its precise microstructural driver---remains a promising but unproven direction.
...
The horizons above stop at thirty minutes; to instrument the six and a half hours that the paper's title invokes, we extend the hidden-execution markout to one hour, two hours, and the burst-to-close mid, and---correcting the coarseness of a uniform-in-time placebo---net each horizon against a \emph{time-of-day-stratified} placebo that draws each burst's counterfactual time within its own 30-minute intraday bucket, so the null preserves the U-shaped intensity and drift profile that a uniform placebo ignores. Streaming a 48-name, price-stratified sample (\$6--\$1{,}758) over 2023 ($10{,}937$ name-days with bursts), Table~\ref{tab:hidden_term} reports the result. Two facts stand out. First, the stratified placebo is \emph{near zero at every horizon} ($|\cdot|\le0.3$ bps out to two hours), materially smaller than the uniform placebo, so the longer-horizon markout is a genuine footprint rather than absorbed intraday drift---which supersedes the more cautious ``drift'' reading of the previous subsection. Second, the net footprint \emph{builds and then persists}: from $+1.9$ bps at three minutes to a plateau of $+2.4$ to $+2.6$ bps from thirty minutes through two hours, and it remains $+2.4$ bps (date-clustered $t=6.3$) measured all the way to the close. A hidden burst's directional information is therefore impounded within minutes and \emph{held} through the trading day, then---as the overnight information coefficient below shows---erased across the overnight auction. This is the paper's cleanest single view of the term structure: a footprint that lives and persists intraday and dies at the overnight boundary. The sample here is a 48-name subset over one year (its three-minute gross markout, $+1.86$ bps, is close to the full-universe $+1.62$); a full-universe, multi-year replication is the natural next step, but the qualitative term structure---genuine, sustained intraday, and absent overnight---is what the title promised to measure.
...
\midrule
3 min    & $+1.86$ & $-0.03$ & $+1.90$ & $+9.4$ \\
15 min   & $+2.43$ & $+0.34$ & $+2.09$ & $+4.6$ \\
30 min   & $+2.36$ & $-0.01$ & $+2.38$ & $+5.5$ \\
1 hour   & $+2.47$ & $-0.09$ & $+2.56$ & $+5.7$ \\
2 hour   & $+2.38$ & $-0.09$ & $+2.48$ & $+6.0$ \\
to close & $+1.64$ & $-0.80$ & $+2.44$ & $+6.3$ \\
\bottomrule
\end{tabular}
\caption{Intraday term structure of the hidden-execution footprint (48-name price-stratified sample, 2023, $10{,}937$ name-days, $\kappa=0$, Lee--Ready signed). Each horizon is netted against a \emph{time-of-day-stratified} placebo (counterfactual times drawn within each burst's own 30-minute bucket). The placebo is near zero at every horizon, so the net footprint is genuine and sustained---not intraday drift; $t$-statistics are date-clustered over the 250 trading days. The footprint persists to the close ($+2.4$ bps) but does not carry across the overnight auction (close-to-open $\mathrm{IC}\approx0$; text).}
...
\label{sec:tugofwar}
Every daily evaluation so far---the overnight relation of Section~\ref{sec:signsign}, the reversal of Section~\ref{sec:reversion}---measures the burst signal against a return that spans one full calendar day. The introduction's motivation, that overnight and intraday returns behave differently \citep{lou2019}, makes the obvious question unavoidable: does that daily return conceal offsetting components? This section splits it. The answer is that it does. The overnight (close-to-open) and intraday (open-to-close) sessions carry \emph{opposite-signed} predictability from the same daily signal, and in the 2017--2021 sample they very nearly cancel---which is why the close-to-close evaluations return a number near zero.

\textbf{Construction.} At the close of day $t$ we form the cross-sectional $z$-score of each name's net signed burst volume (buy minus sell share volume), demean it across names so the book is dollar-neutral, and scale gross exposure to one. Holding that book, we separately accumulate the three returns it can earn: the overnight leg $O_{t+1}/C_t-1$, the intraday leg $C_{t+1}/O_{t+1}-1$, and their compounding, the close-to-close leg $C_{t+1}/C_t-1$. Positions use only information available at the close of $t$. Daily open and close prices are split- and dividend-adjusted from a consolidated daily feed; all $t$-statistics are Newey--West with ten lags on the daily P\&L series, and a positive Sharpe under the ``follow-flow'' convention means the signal predicts \emph{continuation}, a negative one reversal.

\textbf{Samples.} We use two, deliberately independent in both period and signal. \emph{Sample A} is the 2017--2021 aggressive-burst panel: the extraction of Section~\ref{sec:burstdef} run over a point-in-time universe that admits post-2016 IPOs and names that later delist, yielding 493 names with recovered burst data, of which 476 match a daily open/close series, over 1{,}256 trading days. \emph{Sample B} is the 474-name hidden-execution panel of Section~\ref{sec:reconstruction} (470 matched) over 2023--2024, whose signed flow is Lee--Ready-signed type-5 volume rather than aggressive burst flow. Sample B is therefore a replication on a different period \emph{and} a different reconstruction.

\begin{table}[htbp]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Leg (follow-flow convention)} & \textbf{A: 2017--21 aggressive} & \textbf{B: 2023--24 hidden} \\
\midrule
Overnight (close $\to$ open)  & $+2.46$ ($t=+4.83$) & $+2.44$ ($t=+3.36$) \\
Intraday (open $\to$ close)   & $-1.99$ ($t=-4.04$) & $-0.44$ ($t=-0.63$) \\
Close-to-close (their sum)    & $+0.14$ ($t=+0.31$) & $+1.38$ ($t=+1.95$) \\
\bottomrule
\end{tabular}
\caption{Annualized Sharpe ratios of a dollar-neutral book formed on the cross-sectional $z$-score of daily net signed burst volume, by session. Positive = continuation, negative = reversal; Newey--West $t$ (10 lags), gross of costs. The overnight leg replicates across both period and reconstruction; the intraday reversal leg does not. In Sample A the two legs nearly cancel at the daily horizon.}
\label{tab:tugofwar}
\end{table}

\textbf{The overnight leg replicates; the intraday leg does not.} Table~\ref{tab:tugofwar} is the result. Net signed burst volume predicts \emph{continuation} overnight at a Sharpe of $+2.46$ ($t=4.83$) in Sample A, and the hidden-execution panel reproduces it almost exactly---$+2.44$ ($t=3.36$)---on a disjoint two-year window with a different signal construction. The intraday leg is the mirror image in Sample A, a reversal of $-1.99$ ($t=-4.04$), but it does \emph{not} survive replication (Sample B: $-0.44$, $t=-0.63$, and it flips sign between 2023 and 2024). We therefore claim the overnight continuation as a general property of net informed volume and treat the intraday reversal as specific to aggressive flow in the 2017--2021 window. The close-to-close row records the consequence: where both legs are present they offset, and the daily aggregate is indistinguishable from zero ($+0.14$); where only the overnight leg is present it survives to the daily horizon ($+1.38$).

\textbf{What carries the information: magnitude, not ratio.} This decomposition also reconciles a result that would otherwise sit awkwardly against Section~\ref{sec:signsign}, where the close-to-open information coefficient of the \emph{normalized} conditional order imbalance is indistinguishable from zero. That null is not overturned here---it is reproduced. Table~\ref{tab:tugofwar_norm} runs the overnight leg under four normalizations of the same underlying flow. Dividing net flow by the \emph{same day's} total burst volume, the COI construction, removes essentially all of the overnight predictability ($+0.28$, $t=0.46$ in Sample B). Scaling instead by the name's own trailing 20-day average volume---which preserves how large the day's net flow is relative to that name's norm---retains a significant effect in both samples. The overnight information is carried by the \emph{magnitude} of net signed volume, and same-day normalization divides out precisely the quantity that carries it.

\begin{table}[htbp]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Overnight leg, signal normalization} & \textbf{A: 2017--21} & \textbf{B: 2023--24} \\
\midrule
Raw signed volume, cross-sectional $z$        & $+2.46$ ($t=+4.83$) & $+2.44$ ($t=+3.36$) \\
Raw $z$, neutralized within volume quintiles  & $+2.42$ ($t=+4.68$) & $+2.35$ ($t=+3.24$) \\
Net flow / own trailing 20-day volume          & $+1.10$ ($t=+2.25$) & $+1.73$ ($t=+2.78$) \\
Net flow / same-day volume (COI-like)          & $+0.82$ ($t=+1.75$) & $+0.28$ ($t=+0.46$) \\
Sign of net flow only                          & $+1.35$ ($t=+2.77$) & $+0.31$ ($t=+0.46$) \\
\bottomrule
\end{tabular}
\caption{Overnight (close-to-open) leg under alternative normalizations of the same flow. Same-day normalization---the conditional-order-imbalance construction---removes the effect, reproducing the overnight null of Section~\ref{sec:signsign}; scaling by the name's own trailing volume retains it. Neutralizing the raw signal within same-day volume quintiles leaves it essentially intact, so the effect is not a size tilt.}
\label{tab:tugofwar_norm}
\end{table}

\textbf{Robustness of the overnight leg.} Five checks, run on Sample A. (i) \emph{Not a size tilt}: demeaning the signal within same-day volume quintiles leaves the Sharpe at $+2.42$ (Table~\ref{tab:tugofwar_norm}). (ii) \emph{Not earnings gaps}: winsorizing overnight returns at $\pm5\%$ \emph{raises} the Sharpe to $+2.57$ ($t=5.23$), and dropping every name-day whose overnight move exceeds $5\%$ outright still leaves $+2.15$ ($t=4.19$)---the effect is a broad, everyday phenomenon, and the large gaps are noise rather than its source. (iii) \emph{Not an illiquidity artifact}: restricting to the 100 most active names by burst turnover \emph{strengthens} it to $+2.79$ ($t=5.73$), which is also where opening prints are most reliable. (iv) \emph{Not a construction artifact}: permuting the flow signal across names within each day, so the return panel and the marginal distribution of the signal are untouched but the name-to-flow link is broken, gives a mean Sharpe of $-0.00$ across twenty draws (s.d.\ $0.48$); the realized $+2.46$ sits roughly five standard deviations outside that null. (v) \emph{Fresh information only}: replacing day $t$'s flow with day $t-1$'s collapses the Sharpe from $+2.46$ to $+0.30$, and lags two and three are likewise flat. A slow-moving characteristic---size, sector, a persistent liquidity tilt---would predict at all lags; this decays within one day, as an information signal should. The aggregate overnight drift is not the driver either: the equal-weighted overnight return is a large $+8.6$ bps per day in this sample, but the book is dollar-neutral, so that common component nets out and the effect is purely cross-sectional.

\textbf{Economic magnitude and frictions.} The overnight leg fully cycles each day---entry at the close, exit at the open---so it is unusually cost-sensitive for a daily signal. Charging a symmetric per-side cost against the round trip, the Sample A Sharpe declines from $+2.46$ gross to $+1.84$ at $0.5$ bp, $+1.23$ at $1$ bp, $+0.61$ at $1.5$ bp, and reaches zero near $2$ bp per side; Sample B decays on the same schedule. Trading both legs where both exist---holding the flow direction overnight and reversing it intraday---compounds to a gross Sharpe of $+2.95$ ($t=5.48$) and $+7.6$ bps per day in Sample A, retaining $+1.39$ ($t=2.59$, $+3.6$ bps/day) at $1$ bp per side. The effect is thus economically material at institutional execution costs but not at retail ones, and it requires market-on-close and market-on-open execution together with the ability to short the lower leg.

\textbf{What this does and does not establish.} Three limits bound the claim. First, the open and close prices come from a consolidated daily feed rather than from the LOBSTER book; the effect concentrating in the most liquid names and surviving the exclusion of large gaps both argue against a stale-print artifact, but measuring the intraday leg against LOBSTER's own opening and closing prints is the cleaner test and is left to future work. Second, the 2017--2021 LOBSTER archive does not contain every CRSP-listed name-day in the point-in-time universe; missing name-days are flagged and dropped rather than read as zero flow, and because the archive holds no burst data at all for the names that delist within the window, the book takes no position in them---so the result cannot be a delisting-gap artifact, but neither is the sample fully survivorship-free. Third, the intraday reversal leg is a single-sample finding and is reported as such.

The reading we take from this is a refinement of the paper's overnight-efficiency result rather than a reversal of it. Section~\ref{sec:termstructure} shows a hidden-execution footprint that persists through the trading day and is gone by the next morning; Section~\ref{sec:signsign} shows that the normalized imbalance carries no overnight information. Both stand. What this section adds is that the overnight auction is not merely where the intraday footprint is erased---it is also where the component of order-flow information carried by net signed \emph{volume} is impounded into price. The efficiency is real, and it is an overnight event; measuring it close-to-close, against a session that in one sample pushes the other way, is what made it invisible.

\section{Conclusion}
This paper traces the lifecycle of the directional information carried by intraday order-submission bursts, and finds that it is a story of rapid, orderly decay against an efficient overnight market. The central result, established under look-ahead controls enforced architecturally throughout the pipeline and at cross-sectional scale, has a clean term structure. At minutes-scale horizons, informed bursts leave a genuine, measurable footprint: properly signed hidden-execution bursts move price a mean $+1.6$ bps over three minutes across the full 474-name cross-section ($t=3.4$, significant in $79\%$ of names, surviving a matched-random-time placebo that leaves $95\%$ of the effect intact). But that footprint is bounded on the two dimensions that determine tradability---it is \emph{sub-spread} (smaller than the bid--ask spread one must cross to trade it), and it \emph{does not reach the overnight horizon} (close-to-open information coefficient $-0.001$, $t=-0.4$). By the overnight clearing return the directional information has fully decayed; the volume-weighted cross-sectional order-imbalance signal is insignificant (Fama--MacBeth $t\approx-1.4$), any per-name overnight strategy built on it is just an incidental short-market-beta exposure with residual $\alpha$ indistinguishable from zero, and the residual relation that remains is reversal-signed---consistent with liquidity providers absorbing the transient impact and reverting the overshoot (though the volatility state-dependence that inventory models predict is not detected; Section~\ref{sec:baselines}). Informational order flow is thus real and intraday-persistent but sub-spread, priced out with efficiency at the overnight boundary before it could be monetized.

Decomposing that daily horizon into its two sessions (Section~\ref{sec:tugofwar}) refines the last step. The overnight auction is not only where the intraday footprint is erased; it is also where the component of order-flow information carried by net signed \emph{volume} is impounded. A dollar-neutral book formed on the magnitude of daily net burst volume earns an annualized Sharpe of $+2.46$ ($t=4.8$) over the close-to-open session in the 2017--2021 aggressive-burst panel and $+2.44$ ($t=3.4$) in an independent 2023--2024 hidden-execution panel, while the same flow normalized by same-day volume---the conditional-imbalance construction whose overnight null is reported above---carries essentially nothing. The effect is not a size tilt, not driven by earnings gaps, strongest in the most liquid names, absent when the name-to-flow link is permuted, and gone by the following day's lag. It is also cost-sensitive, reaching zero near $2$ bp per side because the position cycles daily. The close-to-close horizon at which the burst signal had been evaluated conceals this, in the 2017--2021 sample because an intraday reversal of nearly equal size pushes the other way.

The measurement rests on two standard safeguards rather than methodological novelties: excluding the forward-looking $D_b$ filter from the predictive feature set and from any evaluation horizon inside its own window, and day-level date-clustered inference. Against these, the paper's contribution is empirical: the cross-sectional measurement of the burst footprint's term structure, and the identification of hidden-execution clustering as the one reconstruction---among Hawkes, order-flow imbalance, and book-resilience alternatives---with a robust informed footprint. Signing the roughly half of hidden prints that execute at the midpoint by tick rule lowers the footprint to $+0.6$ bps but leaves it significant; the effect is real, placebo-robust, and classifier-dependent in magnitude.
...
The natural next steps follow directly from the diagnosis, and two we have already probed on the existing data with pre-specified, day-level tests---reported here as \emph{exploratory} leads rather than confirmed results, since they are in-sample and await the discovery/confirmation split a clean historical backfill would permit. First, redirecting the \emph{target} from signed overnight returns to \emph{volatility}: burst \emph{intensity} (count, not sign) predicts next-day realized volatility and overnight-gap magnitude strongly and \emph{incrementally beyond volatility persistence} (Fama--MacBeth day-level $t\approx6.9$ on the intensity coefficient after controlling for today's absolute return), volatility being far more predictable than direction and monetizable through instruments where the equity spread is irrelevant. Second, \emph{multi-day accumulation campaigns}: defining a campaign as three or more consecutive same-signed daily-flow days, returns in the campaign direction \emph{reverse} significantly in the week after it ends ($-7.5$ bps/day, day-level $t=-4.1$), consistent with post-metaorder decay \citep{bucci2019} and suggesting the tick-constrained reversal is a diluted average of this sharper post-campaign reversion. Extending the sample backward would sharpen both, but is limited by archive coverage: the 2017--2021 LOBSTER pull we examined omits most of the study universe (including two of the four tuning names), so a clean historical extension awaits backfilling those names. Beyond these, the sub-spread, intraday-confined footprint is more naturally a \emph{passive-execution} or trade-scheduling input---where a liquidity provider collects the spread rather than paying it---than a standalone overnight alpha; and cross-asset order-flow lead--lag, on the per-name daily flows already in hand, is the next panel test.
...
\subsubsection{Phase III Formalization: Institutional Execution Architecture}
\label{sec:phase3}
