...
\begin{itemize}
    ...
    \textbf{$D_b$ is an ex-post impact metric, firewalled from prediction by construction.} $D_b$ is, definitionally, the 1--10 minute forward markout of a burst; it is realized only \emph{after} the burst terminates and cannot exist at the prediction instant. We therefore treat it strictly as an instrument for \emph{measuring} realized price impact after the fact, and we impose an architectural firewall that excludes $D_b$ and every quantity derived from it from the predictive online-SGD feature set (Section~\ref{sec:features}). This firewall is enforced in the final pipeline \emph{after} an internal audit revealed that an earlier model had admitted $D_b$-derived features (Appendix~\ref{sec:audit}, row M3, where dropping them moved the mean per-name Sharpe from $-0.28$ to $-0.20$); we report both the violation and its correction rather than present the separation as flawless from the start. Its only two admissible uses are (i) as the ex-post decay measurement in the markout analysis and (ii) as a filter applied \emph{strictly to the trailing training window}, never to a same-day prediction candidate. The firewall is not cosmetic: gating on $D_b$ at any intraday horizon inside its own window is circular look-ahead that manufactures a spurious edge (a $D_b$-gated three-minute markout reads $+8.8$ bps against the honest $+0.5$; Section~\ref{sec:markout}). A live burst ending near the close, moreover, realizes $D_b$ up to ten minutes later---after the 3:50/3:55~PM market-on-close cutoff---so $D_b$ could not be acted upon even if one wished to. Enforcing this separation in the deployed pipeline is what makes the walk-forward results below interpretable.
\end{itemize}
