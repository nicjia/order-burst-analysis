...
Three conclusions follow. First, the effect is genuinely order-flow-based: a plain 1/5/20-day price-reversal baseline is flat-to-negative under this construction, and burst flow retains most of its Sharpe after orthogonalizing to lagged returns (Figure~\ref{fig:oos_pnl}). Second, the apparatus is superfluous, and for \emph{this} construction the signal's \emph{magnitude} adds nothing out of sample: a plain signed-volume signal already delivers the bulk of the result, and a direction-only sign-of-net-flow signal matches or beats the magnitude-weighted headline out of sample ($+1.04$ vs $+0.79$). The qualifier matters, because the session decomposition of Section~\ref{sec:tugofwar} orders the two differently: in the samples where an overnight leg appears at all, the raw magnitude-weighted signal dominates the sign-only variant (Table~\ref{tab:tugofwar_norm}). The two statements are not in conflict---they concern a twenty-day tick-constrained reversal held close-to-close versus a one-night continuation in a different sample---but neither generalizes past its own construction, and we draw no conclusion about magnitude-versus-sign as a property of burst flow in general. Third---and this resolves an apparent puzzle in the table---feeding the SGD prediction in as the signal is not a hidden profitable signal of the opposite sign: the prediction is strongly \emph{anti}-correlated with the burst side (correlation $-0.205$; it agrees with the side on only $21\%$ of the $437$ out-of-sample names, consistent with the Direction-only ablation of Section~\ref{sec:features}), so trading momentum on the prediction is approximately trading reversal on the flow with extra noise---a double negative, not new information. The deployable content is the sign of raw net order flow; the linear ``high-dimensional'' model is not what produces the edge.
...
reported a permanent fraction of $1.01$. That specification carries only $120$ seconds of
...
\bottomrule
\end{tabular}
\caption{Multi-day reversion of the burst-day move (2023--2024 hidden panel, dollar-neutral
cross-sectional sort on signed aggressive-hidden flow). No horizon shows statistically
significant reversion, but the point estimates turn negative from day three and reach
$-8.4$ bps by day twenty---several times the intraday footprint itself. The test is
underpowered and we read it as failing to refute permanence rather than as confirming it.}
\label{tab:multiday}
\end{table}

The honest reading is a qualified one. No horizon out to a month shows significant
reversion, so the permanence claim is not refuted. But the point estimates are negative from
day three onward and reach $-8.4$ bps by day twenty, which is several times the $+1.3$ to
$+2.1$ bps intraday footprint---so a reversion large enough to undo the entire effect sits
comfortably inside the confidence interval. With daily data on a two-year panel this test
cannot do better. We therefore say that the footprint is permanent \emph{within the trading
day}, which is what we measure, and that its fate at multi-day horizons is undetermined
rather than settled; the title's adjective should be read with that scope.

\subsection{Mechanical Quote Displacement: A Test on Pre-Trade Book State}
\label{sub:depletion}

The most serious competing account of the footprint is mechanical rather than
informational. An aggressive hidden execution consumes displayed depth at the touch; if the
level is depleted the best quote moves, the midpoint moves with it, and if the level is
never replenished that displacement persists. Such an account reproduces every stylized
fact reported above---a footprint that appears instantly, never reverts, scales with the
spread, and is largest in coarse-tick cheap names---without any information being conveyed.
Ratio stability across spread quintiles does not discriminate between the two, because it
is equally a prediction of queue geometry.

The test must therefore condition on something known \emph{strictly before} the print. We
use the pre-trade depletion ratio
\[
  d \;=\; \frac{\text{trade size}}{\text{displayed depth at the touch, immediately pre-trade}},
\]
both components of which are observed before any price response. The mechanical account
makes a sharp prediction about $d$: impact is displacement, displacement requires consuming
the queue, and a print that takes a small slice of a deep book cannot move the midpoint at
all. The informational account makes no such prediction---a trade that reveals private
information causes belief revision regardless of how much of the visible queue it happens
to consume.
...
$127{,}667$ name-days). Both the trade size and the displayed depth are measured before the
print, so the conditioning variable cannot be contaminated by the price response. Prints
consuming less than a tenth of the displayed queue---which cannot mechanically displace the
midpoint---still carry $+1.13$ bps, $87\%$ of the all-print average.}
\label{tab:depletion}
\end{table}

The mechanical account fails this test. Prints consuming less than $10\%$ of the displayed
depth---$24\%$ of all aggressive prints, $46$ million of them---carry a footprint of
$+1.134$ bps at $t=26.3$, which is $87\%$ of the unconditional average. These are trades
that leave the touch essentially intact: the level is still there, and still deep, after
the print. There is no displacement to explain the subsequent price move, yet the move is
almost as large as for prints that sweep the level entirely.

The gradient across quintiles tells the same story quantitatively. The depletion ratio rises
by orders of magnitude from Q1 to Q5, while the footprint rises only $22\%$ ($+1.17$ to
$+1.43$). Under a mechanical account impact should be roughly proportional to the fraction
of the queue consumed, which would put Q1 near zero; it is instead within a fifth of Q5.

We do not read this as excluding a mechanical component entirely---the gradient is monotone
and the $d \geq 1.0$ cell is $58\%$ larger than the $d<0.10$ cell, consistent with genuine
displacement adding to an informational base (and with larger, more aggressive orders being
more informed). What the test establishes is that displacement cannot account for the
\emph{level} of the footprint, because the bulk of it survives in prints that displace
nothing. Together with the news-event test of Appendix~\ref{sub:news}, the two leading
non-informational explanations for the footprint are now addressed with data rather than
argument.
