...
The central finding is one of rapid, orderly information decay. Order bursts carry a genuine, measurable informed footprint at minutes-scale horizons: properly (Lee--Ready) signed hidden-execution bursts move price by a mean $+1.6$ bps over three minutes across the full 474-name cross-section (pooled date-clustered $t=3.4$, individually significant in $79\%$ of names, and surviving a matched-random-time placebo that leaves $95\%$ of the effect intact). This footprint is bounded on the two dimensions that determine tradability: it is \emph{sub-spread}---smaller than the bid--ask spread one must cross to trade it---and it \emph{does not reach the overnight horizon} (close-to-open information coefficient $-0.001$, $t=-0.4$). The directional information injected by informed bursts is thus priced within minutes; by the overnight clearing return it has fully decayed, and what residual relation remains is reversal-signed---consistent with liquidity providers absorbing the transient impact and reverting the overshoot.
...
\bottomrule
\end{tabular}
\caption{Full-universe hidden-execution footprint (474 names, 2023--2024, $\kappa=0$, Lee--Ready signed). The three-minute footprint is real and pervasive and survives a matched-random-time placebo (net $+1.58$ bps, $t=4.8$; $95\%$ of the gross markout, see text). The longer-horizon markouts do \emph{not} reverse in the broad cross-section (unlike the two tick-constrained mega-caps of Table~\ref{tab:hidden_horizon}); part of the longer-horizon markout is ordinary intraday drift (the placebo grows with horizon), though $76\%$ still survives at 30 min on the 39-name placebo subsample. Because that subsample matches the universe only at three minutes, we rest the claim on the three-minute figure and treat the longer horizons as suggestive; a full-universe, time-of-day-stratified placebo is the clean resolution.}
