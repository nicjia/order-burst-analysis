...
\begin{abstract}
Hidden liquidity is commonly treated as a single object carrying a single informativeness.
Using event-by-event LOBSTER message data for 474 NASDAQ-traded names over 2023--2024, we show
it is neither, and that the second half of that claim is the more consequential. Hidden
executions away from the midpoint are associated with a positive signed markout while
executions \emph{at} the midpoint carry a small negative one, so any classifier that
force-signs midpoint fills averages an informed population against an uninformed one and
returns approximately zero. That split survives a matched tick-rule placebo and is the
within-venue signature of the self-selection equilibrium of \citet{zhu2014}. Its \emph{level},
however, is not robust: the same footprint measures $+2.09$ bps under the conventions most
commonly adopted, $+0.62$ bps when prints are signed only where the aggressor is unambiguous
and no burst is formed, and zero when burst formation is additionally denied sight of price.
We report $+0.62$ as the conservative level, show that almost all of it is impounded within
one second of the print, and argue that the convention-dependence itself---not the
magnitude---is what the evidence supports.
\end{abstract}
...
carry $-0.42$ bps at three minutes, and more negative values at longer horizons that the
placebo of \S\ref{sec:tickplacebo} shows to be partly artifact. Because the standard trade-sign
...
spread is controlled; and on the \citet{huangstoll1997} accounting it is between $22\%$ and
$51\%$ of the effective half-spread, depending on whether price movement coincident with a
touch sweep is counted as information or as displacement (\S\ref{sec:sweep}); it is incremental to visible order-flow imbalance; and
...
conditions on the prior return path, independently finds the response decaying to zero or
below by ten minutes. We withdraw
...
($-0.42$ at three minutes; the more negative values at fifteen and thirty minutes are
discounted in \S\ref{sec:tickplacebo}, where the placebo recovers $15$--$16\%$ of them
against $5.4\%$ at three minutes), which is the signature of
...
\subsection{The adverse-selection share is a range, not a number}
\label{sec:hsrange}

That $51\%$ is measured from the print, and \S\ref{sec:sweep} shows that most of the level so
measured coincides with the touch being swept within $100$ms. Whether that movement belongs
in the numerator is exactly the question the sweep test could not settle, so the share must
be reported as a range. We re-estimate the decomposition separately on swept and unswept
prints, and with the price-impact leg measured from $t+1$s so that anything coincident with
the touch changing is excluded.

\begin{table}[htbp]
\centering
\begin{tabular}{lccccc}
\toprule
& \textbf{Quoted/2} & \textbf{Effective/2} & \textbf{Impact from $t$} & \textbf{From $t+1$s} & \textbf{Share} \\
\midrule
Swept prints & $2.744$ & $1.845$ & $2.785$ & $0.359$ & $151\%$ / $19.5\%$ \\
Unswept prints & $2.747$ & $1.708$ & $0.397$ & $0.381$ & $23.2\%$ / $22.3\%$ \\
\bottomrule
\end{tabular}
\caption{Huang--Stoll decomposition by sweep group (470 names, 2023--2024, bps). ``Share''
gives price impact as a fraction of the effective half-spread, measured from $t$ and from
$t+1$s respectively. Levels here are estimated under this run's filter and are not directly
comparable to Table~\ref{tab:spread}; the informative content is the \emph{within-run}
contrast between groups and between measurement origins.}
\label{tab:hsrange}
\end{table}

Three things are worth reading off this table. First, the two groups face
\emph{indistinguishable quoted spreads} ($2.744$ against $2.747$), so the difference between
them is not a compensation difference---it is what happens after the print. Second, the swept
group's impact measured from $t$ is $151\%$ of the effective half-spread, which is not a
possible adverse-selection share: it implies a negative realized half-spread, the provider
losing more than they charge. That figure is displacement being counted as information, and
its impossibility is the clearest evidence that the from-$t$ measurement overstates. Third,
once the first second is excluded the two groups converge to $19.5\%$ and $22.3\%$---the same
quantity, as one would expect if what remains is a common informational component.

We therefore report the adverse-selection share of the effective half-spread as
\textbf{between roughly $22\%$ and $51\%$}: $51\%$ if movement coincident with the touch
changing is price discovery, $22\%$ if it is displacement, with the truth in between given
that \S\ref{sec:consvwdraw} shows maker withdrawal is involved in $43\%$ of sweeps. The
qualitative claim---that a substantial share of the effective cost of aggressive hidden
execution is compensation for information rather than provider margin---holds across the
whole range. The point estimate does not.
...
The most serious competing account of the footprint is mechanical rather than informational.
An aggressive execution consumes displayed depth; if the level is depleted the best quote
moves, the midpoint moves with it, and if the level is never replenished that displacement
persists. Such an account reproduces every stylised fact above---a footprint appearing
instantly, never reverting, scaling with the spread, largest in coarse-tick names---without
any information being conveyed.

Testing it requires conditioning on something known strictly \emph{before} the print, and the
natural candidate---the ratio of trade size to displayed depth at the touch---turns out not to
be the right quantity. A type-5 print executes against non-displayed liquidity, so when the
hidden order rests inside the spread the print does not consume the displayed queue at all.
That ratio measures order size relative to visible liquidity, not the fraction of the queue
consumed, and a print that is small on that measure may still be followed microseconds later
by the same aggressor's marketable remainder sweeping the touch. The mechanical channel that
actually applies to hidden prints is that sequential sweep, and we test it directly below.
...
unswept prints per name-day). Both rows separate the conditioning window from the measurement
window; a version measuring from $t$ itself would condition on a quote move inside the
interval it then measures, and is not reported for that reason.}
\label{tab:sweep}
\end{table}
...
\textbf{But the level of the measured footprint is concentrated in the first second.} The
clean rows show that once the displacement window is excluded, swept and unswept prints carry
between $+0.20$ and $+0.44$ bps, against an all-print average measured from the print itself
of roughly $+1.3$. Most of what a from-the-print markout records therefore occurs while the
touch is changing, and we do not claim that displacement can be excluded as its source.

\subsection{What actually happens at the touch: consumption versus withdrawal}
\label{sec:consvwdraw}

Calling all of that ``displacement'' assumes the touch disappears because the aggressor's
marketable remainder consumed it. The flag above does not establish that: it fires when the
touch price moves away \emph{or} its size more than halves, and the second clause captures
cancellations as well as executions. Those are opposite mechanisms. A remainder eating the
queue is mechanical. A maker \emph{withdrawing} quotes fifty milliseconds after a hidden
print is another participant revising beliefs on the information the print revealed---which
is the Glosten--Milgrom mechanism operating, not an artifact of it.

ITCH separates them message by message. For a buy print the touch is the ask, where resting
orders are sell limit orders, so within $(t, t+100\text{ms}]$ at the pre-print ask price we
record whether a type-4 (visible execution) message appears, a type-2/3 (cancel or delete)
message appears, or both.

\begin{table}[htbp]
\centering
\begin{tabular}{lrcc}
\toprule
\textbf{What happened at the touch} & \textbf{Share} & \textbf{Markout from $t$} & \textbf{From $t+1$s} \\
\midrule
Consumption only (execution) & $20.6\%$ & $+3.573\;(56.9)$ & $+0.354\;(7.7)$ \\
Withdrawal only (cancellation) & $28.7\%$ & $+1.842\;(58.5)$ & $+0.249\;(12.0)$ \\
Both & $14.6\%$ & $+3.215\;(68.2)$ & $+0.163\;(4.8)$ \\
Neither (price moved otherwise) & $36.1\%$ & --- & --- \\
\midrule
\emph{Unswept prints} & --- & $+0.397\;(24.2)$ & $+0.381\;(24.1)$ \\
\bottomrule
\end{tabular}
\caption{Decomposition of the sweep flag into taker consumption and maker withdrawal (470
names, 2023--2024, $235{,}940$ name-days; shares are of swept prints; bps, NW $t$ in
parentheses). Withdrawal is involved in $43\%$ of sweeps ($28.7\%$ alone plus $14.6\%$ with
consumption) against $35\%$ involving consumption.}
\label{tab:consvwdraw}
\end{table}

Two things follow, and they cut in opposite directions.

\textbf{The displacement reading was too strong.} Maker withdrawal is involved in $43\%$ of
sweeps and taker consumption in $35\%$; withdrawal alone is the single largest category. A
substantial part of what the previous subsection attributed to mechanical displacement is
therefore liquidity providers responding to the print---price discovery under any reading.
The honest statement is that the majority of the measured level coincides with the touch
changing, and that the touch changes for informational reasons at least as often as for
mechanical ones.

\textbf{But the mechanism does not survive the first second.} Measured from $t+1$s, all four
groups collapse into a band of $+0.16$ to $+0.38$ bps, and the \emph{unswept} group is the
highest of them. Whatever distinguishes a consumed touch from a withdrawn one from an
untouched one, it is fully expressed within a second and predicts nothing thereafter. So
while the interpretation of the level changes, the forward footprint does not: it is
approximately $+0.4$ bps regardless of what happened at the touch.

\subsection{What the level actually is}

Three readings of the same quantity are defensible, and we report the range rather than
choose. If sweep-coincident movement is information, the level is the from-$t$ markout on all
aggressive prints. If it is displacement, the level is the unswept figure. If only
withdrawal-driven sweeps count as information, it lies between. Section~\ref{sec:hsrange}
carries this through to the adverse-selection share, which is the quantity that matters for
interpretation, and reports it as $22\%$--$51\%$ of the effective half-spread accordingly.
What the data do not support is quoting the top of that range unconditionally.
...
\begin{tabular}{lc}
\toprule
\textbf{Signed midpoint move (bps)} & \textbf{Outside the quote} \\
\midrule
...
positive figure means price was already travelling \emph{with} the trade before it arrived.
Restricted to prints executing outside the prevailing quote, where the aggressor's side cannot
be misassigned. A pooled column over all aggressive prints is not reported: there the sign is
assigned against the contemporaneous midpoint, so a just-fallen midpoint mechanically labels a
print a buy, and the resulting pre-drift is a classification artifact.}
\label{tab:preprint}
\end{table}

On prints executing beyond the prevailing quote, where the aggressor's identity cannot be
mistaken, price moves \emph{with} the trade direction before it arrives: $+1.195$ bps over the preceding thirty seconds ($t=35.6$) and
...
\textbf{And this is not an artifact of conditioning at the wrong frequency.} The
decomposition above uses a single thirty-second summary of the prior path, while the VAR
conditions on thirty \emph{one-minute} lags---a richer set at a coarser frequency. Flow
chasing returns at the one-to-thirty-minute scale would produce exactly the observed pattern:
no continuation of thirty-second drift, yet full absorption once the minute-scale path is
conditioned on. We therefore re-run the decomposition against the VAR's own conditioning set,
regressing the forward markout on the thirty preceding one-minute signed returns within each
name-day. It does not absorb the markout either. The raw mean is $+1.051$ ($t=47.9$); the
intercept after thirty one-minute lags is $+0.931$ ($t=48.8$), so the entire minute-scale
path accounts for $0.121$ bps, or $11.5\%$, with a mean within-name-day $R^2$ of $0.35$. The
bridge fails at the VAR's frequency as well as at ours.
...
minute, is barely distinguishable from zero at three, and is negative by ten. The
thirty-minute cell is trim-sensitive---at a $10^4$ trim its $t$ runs between $-1.9$ and
$+1.4$---so the robust statement is decay to zero or below by ten minutes, and we do not
lean on the thirty-minute magnitude. The natural reading---that the VAR conditions on the prior return path while
...
\section{How Much of the Footprint Is the Construction?}
\label{sec:construction}

Everything above measures a burst formed as a run of consecutive same-side prints, where
``side'' is the print's price against the contemporaneous midpoint. Sign therefore enters
\emph{formation}, not only measurement, and that is a circularity we had not isolated: if the
midpoint is drifting, prints land mechanically on one side of it and a run appears without any
clustering of trader intent. The signing checks of \S\ref{sec:signrobust} vary how a print is
\emph{signed}; none of them varies how a burst is \emph{formed}. This section removes the
price-conditioning in two stages and reports what is left.

\subsection{Price-free formation}

We hold the outcome fixed and rebuild the construction. Arm~A is the published rule. Arm~B
forms clusters on inter-arrival timing alone---no sign condition---and signs each print
against the midpoint one millisecond \emph{before} it, which the print cannot have moved.
Arm~C forms clusters the same way but signs only prints executing outside the \emph{pre-print}
quote, where the aggressor's side is unambiguous whatever the midpoint does.

\begin{table}[htbp]
\centering
\begin{tabular}{llccc}
\toprule
& \textbf{Formation / signing} & \textbf{3 min} & \textbf{15 min} & \textbf{30 min} \\
\midrule
A & same-side runs / contemporaneous mid & $+1.482\;(38.1)$ & $+1.441\;(20.5)$ & $+1.416\;(18.3)$ \\
B & time clusters / pre-print mid & $+0.602\;(14.9)$ & $+0.589\;(13.1)$ & $+0.614\;(13.7)$ \\
C & time clusters / outside pre-print quote & $+0.029\;(0.5)$ & $-0.042\;(-0.6)$ & $-0.101\;(-1.0)$ \\
\bottomrule
\end{tabular}
\caption{Burst-level footprint as price-conditioning is removed from the construction (470
names, 2023--2024, $235{,}940$ name-days; bps, NW $t$ in parentheses). Arm~B retains $41\%$ of
Arm~A; Arm~C is statistically zero.}
\label{tab:freeform}
\end{table}

The footprint degrades monotonically as the construction loses sight of price. That is the
signature the circularity hypothesis predicts, and it is not a power problem: Arm~C rests on
$4.6$ million clusters.

\subsection{No construction at all}

Arm~C changes formation \emph{and} signing at once, and its clusters are sparse, so it cannot
attribute the collapse to either. It also sits badly with \S\ref{sec:signrobust}, which reports
$+2.09$ on outside-the-quote prints. We therefore drop clustering entirely and treat every
hidden print as its own observation. Nothing here is formed, so nothing here can inherit
formation circularity; the only remaining choice is how a print is signed.

\begin{table}[htbp]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Signing rule (per print, no clustering)} & \textbf{From $t$} & \textbf{From $t+1$s} \\
\midrule
Contemporaneous midpoint (the rule used above) & $+1.375\;(43.9)$ & $+0.530\;(20.8)$ \\
Outside the pre-print quote only & $+0.621\;(12.4)$ & $+0.097\;(2.1)$ \\
\bottomrule
\end{tabular}
\caption{Per-print footprint under two signing rules, measured from the same base (470 names,
$235{,}940$ name-days; bps, NW $t$). The outside-quote figure is $+0.543$ at fifteen minutes
and $+0.495$ at thirty. Only $41$ of $1{,}604$ hidden prints per day---about $2.5\%$---are
unambiguously signable.}
\label{tab:perprint}
\end{table}

Three things follow, and we prefer to state them than to let the earlier numbers stand.

\textbf{The footprint is real but small.} Cleanly signed and unclustered, it is $+0.621$ bps
at three minutes ($t=12.4$), persisting at $+0.495$ out to thirty. It is not zero, so Arm~C
was too pessimistic---clustering plus the requirement of two outside-quote prints per cluster
destroyed a genuine signal.

\textbf{It is roughly one third of the headline.} Holding the measurement base fixed and
changing only the signing rule moves the per-print footprint from $+1.375$ to $+0.621$. The
$+2.09$ of Table~\ref{tab:decomp} rests on three choices that each push the same way:
contemporaneous-mid signing, price-conditioned formation, and measurement from the burst-end
midpoint. We therefore report $+0.62$ as the conservative level and treat $+2.09$ as an upper
bound obtained under conventions that inflate it.

\textbf{Almost all of it is instantaneous.} Measured from one second after the print, the
cleanly-signed footprint is $+0.097$ ($t=2.1$)---statistically marginal and economically
nothing. This is consistent with \S\ref{sec:sweep} and \S\ref{sec:consvwdraw}, and it means the
object being measured is a repricing that happens at the moment of execution, not a drift
that follows it.

\subsection{What is actually robust here}

The stable finding across this section is not a magnitude. It is that the measured
informativeness of hidden liquidity varies by a factor of three---from $+2.09$ to $+0.62$, and
to a statistical zero under Arm~C---according to conventions that the literature does not
usually report: whether midpoint prints are signed or abstained on, whether the midpoint used
is contemporaneous or lagged, whether burst formation conditions on side, and whether the
clock starts at the print or a second later. Every one of those is a defensible choice. None
of them is innocuous.

We regard that convention-dependence, together with the aggressive-versus-midpoint split it
cannot explain away, as this paper's contribution, and the level as provisional.
...
identifiable aggressor are associated with a positive signed midpoint move, flat from three
minutes to the close; executions at the midpoint carry a small negative one. The \emph{level}
of the first is convention-dependent---$+2.09$ bps as conventionally measured, $+0.62$ when
signed unambiguously without forming a burst (\S\ref{sec:construction})---and we quote the
lower figure. Any classifier that
...
The aggressive footprint behaves as an adverse-selection quantity, though its level depends
on a classification we cannot make cleanly. It is roughly one third
...
controlled; it is between $22\%$ and $51\%$ of the effective half-spread on the Huang--Stoll
accounting, depending on the treatment of sweep-coincident movement; it is
